  ...
  Widget build(BuildContext context) {
    return KeepAliveClientMixin(
      child: StreamBuilder<List<TransactionWallet>>(
        stream: database.getAllPinnedWallets(HomePageWidgetDisplay.AllSpendingSummary).$1,
        builder: (context, snapshot) {
          if (snapshot.hasData || appStateSettings["allSpendingSummaryAllWallets"] == true) {
            List<String>? walletPks = (snapshot.data ?? []).map((item) => item.walletPk).toList();
            if (walletPks.length <= 0 || appStateSettings["allSpendingSummaryAllWallets"] == true) walletPks = null;
            return Padding(
                    ...
                    child: TransactionsAmountBox(
                      ...
                      openPage: TransactionsSearchPage(
                        initialFilters: SearchFilters().copyWith(
                          dateTimeRange: getDateTimeRangeForPassedSearchFilters(cycleSettingsExtension: "AllSpendingSummary"),
                          walletPks: walletPks ?? [],
